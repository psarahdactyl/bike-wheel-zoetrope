...
root_dir = raw_input("Please give the root directory: ")
...
while not isObjDone:
    key_obj = raw_input("Give name of a key object (type 'done' if all objects entered): ")
    if key_obj == 'done':
        isObjDone = 1
    else:
...
for obj in key_objects:
    ...
    for sound in glob.glob(root_dir + '/audio/' + obj + '/*.wav'):
        frame_window = raw_input('During what frames should ' + sound +
                                 ' be heard? (input as x-x, where x is a frame number): ')
        split = frame_window.split("-")
        first_frame = split[0]
        last_frame = split[1]

        obj_audio.append([first_frame, last_frame, sound])
...
for obj in key_objects:
    ...
    for frame in frame_images:
        coords = match(key_object_frame_dirs[obj_count] + '/frame' +
                       str(frame_count + 1) + '.png',
                       frame_images[frame_count])

        obj_values[str(frame_count + 1)] = coords
        frame_count += 1
